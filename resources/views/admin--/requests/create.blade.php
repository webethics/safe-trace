...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('global.request.title_singular') }}
    </div>
        ...
        <form action="{{ route("admin.requests.store") }}" method="POST" enctype="multipart/form-data">
			...
			<div class="form-group {{ $errors->has('other_info') ? 'has-error' : '' }}">
			 ...
			 <div class="form-group {{ $errors->has('priority') ? 'has-error' : '' }}">
                ...
                <input type="radio" id="priority" name="priority" class="form-control" value="normal">{{ trans('global.request.fields.normal') }} <span>{{ trans('global.request.fields.5_10_wokring_days') }}</span>
                
				<input type="radio" id="priority" name="priority" class="form-control" value="urgent">{{ trans('global.request.fields.urgent') }} <span>{{ trans('global.request.fields.asap') }}</span>
			...
			<div class="form-group {{ $errors->has('data_archive') ? 'has-error' : '' }}">
                ...
                <input type="checkbox"  name="data_archive[]" class="form-control" value="screenshot">{{ trans('global.request.fields.screenshot') }}
                
				<input type="checkbox"  name="data_archive[]" class="form-control" value="fullhtml">{{ trans('global.request.fields.fullpagehtml') }}
